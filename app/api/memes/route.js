...
export const POST = async (request) => {
	try {
		const newMeme = await request.json();

		const memesData = JSON.parse(fs.readFileSync(memesFilePath, 'utf8'));

		const updatedMemes = [...memesData.memes, { ...newMeme, id: memesData.memes.length + 1 }];

		const updatedData = { memes: updatedMemes };

		fs.writeFileSync(memesFilePath, JSON.stringify(updatedData, null, 2));

		return NextResponse.json(updatedMemes[updatedMemes.length - 1], { status: 201 });
	} catch (error) {
		console.error('Error adding meme:', error);
		return NextResponse.json({ error: 'Failed to add meme' }, { status: 500 });
	}
};
